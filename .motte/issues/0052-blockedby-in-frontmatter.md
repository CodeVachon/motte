...
blockedBy: [ids] added to the frontmatter schema, emitted as a flow array after labels. Sorted and de-duplicated on write, so a merge that appends the same blocker twice or in a different order converges on one canonical form instead of churning the diff.
